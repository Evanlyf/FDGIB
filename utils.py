...
def load_bail(dataset, sens_attr="WHITE", predict_attr="RECID", path="./dataset/bail/", label_number=1000):
    ...
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        ...
                        dtype=np.float32)  
    adjs = sp.tril(adj, -1)
    adjs = adjs + adjs.T + sp.eye(adjs.shape[0])

    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)
    adj = adj + sp.eye(adjs.shape[0])
...
def load_pokec(dataset, sens_attr="region", predict_attr="I_am_working_in_field", path="./dataset/pokec/",
               label_number=1000):
    # print('Loading {} dataset from {}'.format(dataset, path))
    idx_features_labels = pd.read_csv(os.path.join(path, "{}.csv".format(dataset)))
    header = list(idx_features_labels.columns)
